...
println("Matrices undergoing nmf...")
#nmf_output =nnmf(nmf_input_matrix, 6; alg=:multmse, maxiter=5000, tol=1.0e-4)
...
#write_to_CSV(nmf_output.H,"factors_matrix7.csv")

#write_to_CSV(nmf_input_matrix,"input_matrix_no_background.csv")
...
for i = 1:10
    nmf_output =nnmf(nmf_input_matrix, i; alg=:multmse, maxiter=5000, tol=1.0e-4)
    reconstructed_matrix = nmf_output.W * nmf_output.H
    filename = "sqrt_with_background_reconstructed_matrix_rank" * string(i) *(".csv")
    println("Writing file " * filename)
    write_to_CSV(reconstructed_matrix,filename)
end
